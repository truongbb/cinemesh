...
public interface ShowtimeRepository extends JpaRepository<ShowTimeEntity, UUID>, JpaSpecificationExecutor<ShowTimeEntity> {
    ...
    @Query("select s from ShowTimeEntity s where :startShow between s.startTime and s.endTime or :endShow between s.startTime and s.endTime")
    List<ShowTimeEntity> findTimeIntervalOverlapping(LocalDateTime startShow, LocalDateTime endShow);
